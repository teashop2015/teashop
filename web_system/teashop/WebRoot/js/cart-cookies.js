...
var CartCookie = {
		...
		jsonObj:{},
		getJsonObj : function() {
			if ($.cookie('cartdata') == undefined || $.cookie('cartdata') == 'null') {
		...
		delCartProduct : function(id) {
			...
			for (var i in products) {
				if (products[i].id == id) {
					this.setCartTotalCount(this.getCartTotalCount() - products[i].count);
					this.setCartTotalPay(this.getCartTotalPay() - products[i].count * products[i].price);
					products.splice(i, 1);
